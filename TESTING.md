...
### 3. Session Management Tests (`session.test.js`)
...
- ✅ Prefer header over cookies
- ✅ Return null when no token present
...
#### Session Cookies
- ✅ Set cookies with correct options
...
The project includes a robust JWT-based session management system:
...
#### 3. **Session Service** (`src/services/sessionService.js`)
   ...
   - `extractToken()`: Extract token from request (header/cookies)
   - `validateSession()`: Validate access token
   - `isTokenExpiringSoon()`: Check if token needs refresh
   - `getTokenExpiryTime()`: Get remaining token lifetime
   - `setSessionCookies()`: Set tokens in response cookies
   - `clearSessionCookies()`: Clear session cookies

#### 4. **Enhanced Auth Middleware** (`src/middlewares/enhancedAuthMiddleware.js`)

   Available middleware:
   - `authMiddleware`: Required authentication
   - `optionalAuthMiddleware`: Optional authentication
   - `autoRefreshTokenMiddleware`: Auto-refresh expiring tokens
   - `checkSessionMiddleware`: Check session validity without blocking
...
```javascript
const { authMiddleware, autoRefreshTokenMiddleware } = require('../middlewares/enhancedAuthMiddleware');

router.get('/protected', authMiddleware, (req, res) => {
  // req.user contains authenticated user
  // req.tokenExpiresIn contains remaining time
